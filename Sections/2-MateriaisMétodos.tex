...
Ainda na primeira análise, é de suma importância conhecer as condições iniciais do sistema antes de iniciar os testes. A Tabela \ref{tab:pertubaçõesTransienteCondiçõesIniciais} apresenta as condições iniciais do sistema antes de cada perturbação. Essas condições são essenciais para a análise do comportamento transiente do sistema para correta interpretação da carga térmica e desempenho do sistema de condicionamento de ar.
...
\begin{tabular}{|c|c|c|c|}
    \hline
    \textbf{Perturbação} & 
    \makecell{\textbf{Temperatura Inicial} \\ \textbf{Entrada do} \\ \textbf{Evaporador [°C]}} &
    \makecell{\textbf{Umidade Inicial} \\ \textbf{Entrada do} \\ \textbf{Evaporador [\%]}} &
    \makecell{\textbf{Temperatura Inicial} \\ \textbf{Entrada do} \\ \textbf{Condensador [°C]}} \\
        \hline
        P1 & 33 & 28 & 53 \\
        P2 & 35 & 24   & 55\\
        P3 & 39 & 19  & 66 \\
        \hline
    \end{tabular}
    \caption{Condições Iniciais das Perturbações Geradas no Sistema de Condicionamento de Ar para Análise do Transiente}
    \vspace{5pt} 
{\footnotesize Fonte: O Autor (2025) }
...
Também se faz necessário conhecer as condições iniciais do sistema antes de iniciar os testes da segunda análise. A Tabela \ref{tab:pertubaçõesHistereseCondiçõesIniciais} apresenta as condições iniciais do sistema antes de cada perturbação. Essenciais da mesma forma que as condições iniciais da primeira análise para correta comparação do comportamento do sistema.
...
\begin{tabular}{|c|c|c|c|}
    ...
    \makecell{\textbf{Temperatura Inicial} \\ \textbf{Entrada do} \\ \textbf{Condensador [°C]}} \\
        ...
        \hline
    \end{tabular}
    \caption{Condições Iniciais das Perturbações Geradas no Sistema de Condicionamento de Ar para Análise da Histerese}
    \vspace{5pt} 
{\footnotesize Fonte: O Autor (2025) }
...
\begin{tikzpicture}
  \begin{axis}[
      ...
      axis lines=none,
      axis on top,
      ticks=none % remove números e marcas
    ]
    % Curvas
    \addplot[name path=curveA, thick] {exp(-x)};
    \addplot[name path=curveB, thick] {exp(-2*x)};
    
    % Preencher área entre as curvas
    \addplot[blue!20] fill between[of=curveA and curveB];

    % Retas tracejadas
    \addplot[dashed] coordinates {(0,0) (0,5)}; % x=0
    \addplot[dashed] coordinates {(5,0) (5,5)}; % x=5
    \addplot[dashed] coordinates {(0,0) (5,0)}; % y=0
    \addplot[dashed] coordinates {(0,1) (5,1)}; % y=1
  \end{axis}
\end{tikzpicture}
    }
